...
@WebServlet("/inbox/Inbox.Apiforbackstage")
public class InboxApiforbackstage extends HttpServlet {
...
//		response.setContentType("text/html; charset=UTF-8");
		...
		List<MemberVO> mems = service.getAll();
		Set<String>mem_IDs = new HashSet<String>();
		for(MemberVO mem : mems) {
			mem_IDs.add(mem.getMember_ID());
		}
		System.out.println("all mem account:" + mem_IDs);
...
//轉換String type JSON -->  JSON OBJECT
				...
				String  member_id= jobj.getString("name");
				String title = jobj.getString("title");
				String message = jobj.getString("message");
				String emp_id = jobj.getString("id");
				String state = jobj.getString("state");
				
				System.out.println(member_id + title + message);
...
//驗證資料		
				if(member_id == null || member_id.length() == 0) {
					...
					String result = (mem_IDs.contains(member_id) == true) ? "OK": "NG";  //確認有員工的話傳送ＯＫ否則ＮＧ
					if("NG".equals(result)) {
